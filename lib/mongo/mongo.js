 ...
 */
// TODO Pliman Level2 处理选项
db.find = function (collection, entity, skip, limit, cb) {
	if (!cb) {
		cb = skip;
		skip = 0;
		limit = limit || 10;
	...
	try {
		pool.acquire(function (err, client) {
			...
			new mongodb.Collection(client, collection).find(entity).skip(skip).limit(limit, function (err, r) {
				if (err) {
 ...
 */
// TODO Pliman Level2 处理选项
db.findOne = function (collection, entity, skip, limit, cb) {
	if (!cb) {
		cb = skip;
		skip = 0;
		limit = limit || 10;
	...
	try {
		pool.acquire(function (err, client) {
			...
			new mongodb.Collection(client, collection).findOne(entity).skip(skip).limit(limit, function (err, r) {
				if (err) {
					...
					logger.info("[findOne] find one %s toArray start", uuid);
					r.toArray(function (err, r) {
						if (err) {
							logger.error("[findOne] find one %s toArray failed,err is %s", uuid, err);
						} else {
							logger.info("[findOne] find one %s toArray successfully", uuid);
						}
