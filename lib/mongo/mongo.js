...
var pool = generic_pool.Pool({
	...
	create: function (callback) {
		try {
			var db = new mongodb.Db(mongoDBConf.dbName, new mongodb.Server(mongoDBConf.host, mongoDBConf.port, {}), {
				w: 1
			});

			db.open(function (err, client) {
				db.authenticate(mongoDBConf.userName, mongoDBConf.password, function (err, res) {
					if (err) {
